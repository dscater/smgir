...
@section('css')
<link rel="stylesheet" href="{{asset('css/vistas/gaems/reportes.css')}}">
@endsection
...
@section('content')
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0 text-dark">Administración de GAEM</h1>
                </div>
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item"><a href="{{ route('home') }}">Inicio</a></li>
                        <li class="breadcrumb-item"><a href="{{ route('gaems.index') }}">GAEM</a></li>
                        <li class="breadcrumb-item active">Reportes de GAEM</li>
                    </ol>
                </div>
            </div>
        </div>
    ...
    <section class="content">
                            ...
                            <table class="table table-bordered">
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>{{ $gaem->base->nombre }}</td>
                                    </tr>
                                    <tr>
                                        <td class="font-weight-bold bg-teal celdas_info">Macro Distrito:</td>
                                        <td>{{ $gaem->macrodistrito->nro_macrodistrito }} -
                                            {{ $gaem->macrodistrito->nombre }}</td>
                                    </tr>
                                    <tr>
                                        <td class="font-weight-bold bg-teal celdas_info">Distrito:</td>
                                        <td>{{ $gaem->distrito->nro_distrito }}</td>
                                    </tr>
                                    <tr>
                                        <td class="font-weight-bold bg-teal celdas_info">Ubicación:</td>
                                        <td><a href="{{ $gaem->ubicacion_url }}" target="_blank">{{ $gaem->ubicacion_url }}</a> <img
                                                src="{{ asset('imgs/gaems/' . $gaem->ubicacion_img) }}" alt=""></td>
                                    </tr>
                                    <tr>
                                        <td class="font-weight-bold bg-teal celdas_info">Fecha Inicio:</td>
                                        <td>{{ $gaem->fecha_inicio }}</td>
                                    </tr>
                                    <tr>
                                        <td class="font-weight-bold bg-teal celdas_info">Fecha Fin:</td>
                                        @if((int)$gaem->avance == 100)
                                        <td id="respuesta_estado">{{ $gaem->fecha_fin }}</td>
                                        @else
                                        <td id="respuesta_estado">{{ $gaem->estado }}</td>
                                        @endif
                                    </tr>
                                </tbody>
                            </table>
                ...
                <div class="col-md-12">
                    <div class="row" id="contenedorReportes">
                        <div class="col-md-12 oculto" id="contenedor_formulario">
                            <div class="card">
                                <div class="card-body">
                                    {{ Form::open(['route' => 'gaems.index', 'method' => 'post', 'files' => true, 'id' => 'form_nuevo']) }}
                                    <div class="row">
        ...
        <input type="hidden" id="urlStoreReporte" value="{{ route('gaem_reportes.store', $gaem->id) }}">
        <input type="hidden" id="urlReportes" value="{{ route('gaem_reportes.index', $gaem->id) }}">
    </section>
...
@section('scripts')
    ...
    <script src="{{ asset('js/vistas/gaems/reportes.js') }}"></script>
@endsection
