...
@section('css')
<link rel="stylesheet" href="{{asset('css/vistas/obras/reportes.css')}}">
@endsection
...
@section('content')
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0 text-dark">Administración de Obras</h1>
                </div>
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item"><a href="{{ route('home') }}">Inicio</a></li>
                        <li class="breadcrumb-item"><a href="{{ route('obras.index') }}">Obras</a></li>
                        <li class="breadcrumb-item active">Reportes de Obras</li>
                    </ol>
                </div>
            </div>
        </div>
    ...
    <section class="content">
                            ...
                            <table class="table table-bordered">
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>{{ $obra->base->nombre }}</td>
                                    </tr>
                                    <tr>
                                        <td class="font-weight-bold bg-teal celdas_info">Macro Distrito:</td>
                                        <td>{{ $obra->macrodistrito->nro_macrodistrito }} -
                                            {{ $obra->macrodistrito->nombre }}</td>
                                    </tr>
                                    <tr>
                                        <td class="font-weight-bold bg-teal celdas_info">Distrito:</td>
                                        <td>{{ $obra->distrito->nro_distrito }}</td>
                                    </tr>
                                    <tr>
                                        <td class="font-weight-bold bg-teal celdas_info">Ubicación:</td>
                                        <td><a href="{{ $obra->ubicacion_url }}" target="_blank">{{ $obra->ubicacion_url }}</a> <img
                                                src="{{ asset('imgs/obras/' . $obra->ubicacion_img) }}" alt=""></td>
                                    </tr>
                                    <tr>
                                    ...
                                    <tr>
                                        ...
                                        @if((int)$obra->avance == 100)
                                        <td id="respuesta_estado">{{ $obra->fecha_fin }}</td>
                                        @else
                                        <td id="respuesta_estado">{{ $obra->estado }}</td>
                                        @endif
                                    </tr>
                                </tbody>
                            </table>
                ...
                <div class="col-md-12">
                    <div class="row" id="contenedorReportes">
                        <div class="col-md-12 oculto" id="contenedor_formulario">
                            <div class="card">
                                <div class="card-body">
                                    {{ Form::open(['route' => 'obras.index', 'method' => 'post', 'files' => true, 'id' => 'form_nuevo']) }}
                                    <div class="row">
        ...
        <input type="hidden" id="urlStoreReporte" value="{{ route('obra_reportes.store', $obra->id) }}">
        <input type="hidden" id="urlReportes" value="{{ route('obra_reportes.index', $obra->id) }}">
    </section>
...
@section('scripts')
    ...
    <script src="{{ asset('js/vistas/obras/reportes.js') }}"></script>
@endsection
