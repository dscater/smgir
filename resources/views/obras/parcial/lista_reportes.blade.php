@if (count($reportes) > 0)
    @foreach ($reportes as $reporte)
        <tr data-fecha={{ $reporte->fecha_fin }}>
            ...
            <td data-id="{{ $reporte->user->id }}">
                <ul class="" style="padding-left:5px;">
                    @foreach ($reporte->tecnicos as $tecnico)
                        <li data-id="{{ $tecnico->id }}" data-url="{{ route('obra_tecnicos.destroy', $tecnico->id) }}">
            ...
            <td><img src="{{ asset('imgs/obras/' . $reporte->fotografia) }}" alt=""></td>
            ...
            <td class="btns-opciones">
                <a href="{{ route('obra_reportes.update', $reporte->id) }}" class="modificar"><i
                        class="fa fa-edit" data-toggle="tooltip" data-placement="left" title="Modificar"></i></a>
                <a href="#" data-url="{{ route('obra_reportes.destroy', $reporte->id) }}"
