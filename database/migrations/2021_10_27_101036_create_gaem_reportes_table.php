...
class CreateGaemReportesTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('gaem_reportes', function (Blueprint $table) {
            $table->id();
            $table->bigInteger('nro')->unsigned();
            $table->bigInteger('gaem_id')->unsigned();
            ...
            $table->foreign('gaem_id')->references('id')->on('gaems')->ondelete('no action')->onupdate('cascade');
    ...
    {
        Schema::dropIfExists('gaem_reportes');
    }
}
